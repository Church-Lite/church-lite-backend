...
@RestController
public class CellOrganizationUnitHandlerImpl extends CellOrganizationUnitHandler {
    ...
    public CellOrganizationUnitDTO update(CellOrganizationUnitDTO dto, UUID id) {
        ...
        var entity = dtoConverter.toEntity(dto, null); entity.setId(id); entity.setChildren(current.getChildren());
        entityManager.merge(entity); return dtoConverter.toDTO(entity, null);
    }

    @Override @Transactional
    public CellOrganizationUnitDTO save(CellOrganizationUnitDTO dto) { validateParent(dto, null); return super.save(dto); }
    ...
    public void delete(UUID id) {
        ...
        if (unit.getChildren() != null && !unit.getChildren().isEmpty()) throw new ServiceException(HttpStatus.CONFLICT, "A unidade possui subunidades e não pode ser excluída");
        super.delete(id);
    }
    ...
    private void validateParent(CellOrganizationUnitDTO dto, UUID id) {
        ...
        UUID cursor = dto.getParentUnit().getId(); var visited = new HashSet<UUID>();
        while (cursor != null) {
            if (cursor.equals(id) || !visited.add(cursor)) throw new ServiceException(HttpStatus.CONFLICT, "A hierarquia não pode possuir ciclos");
