...
import com.smartverse.churchlitebackend.dashboard.DashboardModels.*;
...
public class DashboardService {
    ...
    public FinancialSnapshot financial(FinancialFilter requested) {
        ...
        FinancialFilter filter = new FinancialFilter(start, end, requested.bancoId(), requested.contaBancariaId(),
    ...
    private boolean matches(TransactionsEntity t, FinancialFilter f, Set<UUID> openCashIds, boolean dates) {
        ...
        if (f.centroCustoId() != null && (financial.getCostCenter() == null || !f.centroCustoId().equals(financial.getCostCenter().getId()))) return false;
    ...
    private List<CostCenterExpense> costCenters(List<TransactionsEntity> values, double totalExpense) {
        ...
        if (unclassified > 0) result.add(new CostCenterExpense(null, "Sem centro de custo", unclassified, percent(unclassified, totalExpense)));
    ...
    private List<PlanAccountExpense> planAccounts(List<TransactionsEntity> values, double totalExpense) {
        ...
        List<PlanAccountExpense> result = new ArrayList<>(expenses.stream()
                ...
                .entrySet().stream().map(e -> {
                    PlanAccountEntity p = e.getKey(); String code = p.getCodeTree();
        ...
        if (unclassified > 0) result.add(new PlanAccountExpense(null, "", "Sem classificação", 0, null, unclassified, percent(unclassified, totalExpense)));
    ...
    private List<RecentTransaction> recent(List<TransactionsEntity> values) {
        return values.stream().sorted(Comparator.comparing(TransactionsEntity::getDateTransaction).reversed()).limit(8).map(t -> {
            FinancialEntity f = t.getFinancial(); CashEntity c = f.getCash();
    ...
    private List<Alert> alerts(List<TransactionsEntity> values, List<CashTransactionsEntity> sessions) {
        ...
        if (!open.isEmpty()) alerts.add(new Alert("CAIXA_ABERTO", "ATENCAO", "Existem caixas aguardando fechamento.", open.size(), open.getFirst().getId()));
        long withoutCost = values.stream().filter(t -> t.getFinancial().getCostCenter() == null).count();
        if (withoutCost > 0) alerts.add(new Alert("SEM_CENTRO_CUSTO", "INFORMACAO", "Movimentações sem centro de custo.", (int) withoutCost, null));
        long withoutPlan = values.stream().filter(t -> t.getFinancial().getPlanAccount() == null).count();
        if (withoutPlan > 0) alerts.add(new Alert("SEM_PLANO_CONTA", "INFORMACAO", "Movimentações sem plano de contas.", (int) withoutPlan, null));
        return alerts;
    }
    ...
    private double total(List<TransactionsEntity> values, TypeFinancial type) { return values.stream().filter(t -> type(t) == type).mapToDouble(t -> number(t.getValue())).sum(); }
    private double signedTotal(List<TransactionsEntity> values) { return values.stream().mapToDouble(t -> type(t) == TypeFinancial.EXPENSE ? -number(t.getValue()) : number(t.getValue())).sum(); }
    private TypeFinancial type(TransactionsEntity t) { return t.getFinancial().getTypeFinancial(); }
    private double number(Double value) { return value == null ? 0 : value; }
    private boolean between(LocalDate value, LocalDate start, LocalDate end) { return !value.isBefore(start) && !value.isAfter(end); }
    private Double variation(double current, double previous) { return previous == 0 ? null : ((current - previous) / Math.abs(previous)) * 100; }
    private double percent(double value, double total) { return total == 0 ? 0 : value * 100 / total; }
    private String situation(double value) { return value > 0 ? "POSITIVO" : value < 0 ? "NEGATIVO" : "ZERADO"; }
    private String maskAccount(String number, String digit) { if (number == null || number.isBlank()) return null; String visible = number.length() <= 4 ? number : number.substring(number.length() - 4); return "•••• " + visible + (digit == null ? "" : "-" + digit); }
}
